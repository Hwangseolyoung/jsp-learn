...
-- 게시물별 댓글 수 조회(단, 댓글이 없는 개시물은 나오지 않음
SELECT b.id, b.title, b.body, b.inserted, Count(r.id)
FROM Board b INNER JOIN Reply r
ON b.id = r.board_id 
GROUP BY b.id
ORDER BY 1
;

SELECT b.*, Count(r.id) -- b.* : 특정 테이블의 모든 컬럼을 다선택
FROM Board b INNER JOIN Reply r
ON b.id = r.board_id 
GROUP BY b.id
ORDER BY 1
...
-- LEFT OUTER JOIN(교집합 + 왼쪽 테이블 모두 다)
...
-- RIGHT OUTER JOIN (교집합 + 오른쪽 테이블 모두 다)
SELECT * FROM Board b RIGHT OUTER JOIN Reply r ON b.id = r.board_id;
SELECT * FROM Reply r RIGHT OUTER JOIN Board b ON b.id = r.board_id; -- 컬럼순서 달라짐
SELECT * FROM Board b RIGHT JOIN Reply r ON b.id = r.board_id; -- OUTER 생략가능
